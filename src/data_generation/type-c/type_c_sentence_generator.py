...
OUTPUT_CSV = os.path.join(BASE_DIR, "../../data/type-c/sentences_c.csv")
...
def generate_dataset(limit: int = 5000, output_file: str = OUTPUT_CSV) -> None:
    ...
    with open(output_file, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["id", "notation", "sentence"])

        for i, board in enumerate(boards[:limit], start=1):
            writer.writerow([
                i,
                to_notation(board),
                board_to_sentence(board)
            ])

    print(f"Generated {min(limit, len(boards))} samples")
    print(f"CSV saved to: {output_file}")


if __name__ == "__main__":
    generate_dataset(limit=5000)